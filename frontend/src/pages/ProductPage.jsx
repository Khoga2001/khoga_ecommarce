...
import { Minus, Plus, ChevronLeft, ChevronRight, Check, Heart } from 'lucide-react';
...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    wishlistApi.get()
      .then((res) => setIsWishlisted((res.data.product_ids || []).includes(product.id)))
      .catch(() => setIsWishlisted(false));
  }, [user, product?.id]);

  const handleWishlistToggle = async () => {
    if (!user) {
      navigate('/login', { state: { from: `/products/${handle}` } });
      return;
    }
    ...
    try {
      ...
      toast.success(res.data.added ? t('added_to_wishlist') : t('removed_from_wishlist'));
    } catch {
  ...
  return (
    <main className="product-page" data-testid="product-page">
      ...
      {!isSpecialMinimalProduct && <ProductReviews productId={product.id} />}
