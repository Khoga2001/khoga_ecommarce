  ...
  return (
    <main className="collection-page" data-testid="collection-page">
      ...
      <div className="collection-header">
        ...
        <span className="collection-count">{sorted.length} products</span>
      </div>
      ...
      {loading ? (
        <div style={{ padding: '60px 24px', textAlign: 'center', color: '#6b6b6b' }}>Loading…</div>
      ) : sorted.length > 0 ? (
      ...
      <section className="related-collections">
        <h3 className="related-title">Explore More</h3>
          ...
          {allCategories
            ...
            .map(col => (
              <Link key={col.id} to={`/collections/${col.handle}`} className="related-collection-card">
                ...
                <span className="related-col-title">{col.title}</span>
              </Link>
            ))}
        </div>
      </section>
