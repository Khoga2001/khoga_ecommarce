...
import { Package, MapPin, User as UserIcon, LogOut, LayoutDashboard, CreditCard, Plus, X, EarOff, MessageCircle } from 'lucide-react';
  ...
  return (
    <main className="account-page" data-testid="account-page">
      ...
      <div className="account-grid">
        ...
        <div className="account-card" onClick={() => setActiveModal('profile')} style={{ cursor: 'pointer' }}>
          <UserIcon size={28} />
          <div>
      ...
      {activeModal === 'profile' && (
        <div className="modal-overlay" onClick={() => setActiveModal(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginTop: '24px', textAlign: lang === 'ar' ? 'right' : 'left' }}>
              <p><strong>{t('label_name')}</strong> {user.name}</p>
              <p><strong>{t('label_email')}</strong> {user.email}</p>
              <p><strong>{t('label_phone')}</strong> <span dir="ltr" style={{display: 'inline-block'}}>{user.phone || t('not_provided')}</span></p>
            </div>
          </div>
        </div>
      )}
